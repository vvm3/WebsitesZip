...
const policies = [
    ...
    },
    {
        ...
        descriptionLine: "The Website Owner, including subsidiaries and affiliates",
        description: `We provides the information contained on the website or any of the pages comprising the website (“website”) to visitors (“visitors”) (cumulatively referred to as “you” or “your” hereinafter) subject to the terms and conditions set out in these website terms and conditions, the privacy policy and any other relevant terms and conditions, policies and notices which may be applicable to a specific section or module of the website.
...
We as a merchant shall be under no liability whatsoever in respect of any loss or damage arising directly or indirectly out of the decline of authorization for any Transaction, on Account of the Cardholder having exceeded the preset limit mutually agreed by us with our acquiring bank from time to time.`,
    },
    {
        ...
        descriptionLine: "Policy related to cancellation and refund",
        description: `No cancellations & Refunds are entertained for the payments made to the SERATEK SYSTEMS PVT LTD.`
    },
];
...
export default function PolicyDetailsPage() {
    ...
    const activeId = searchParams?.get("id") ?? ""; // ✅ safe fallback

    // ✅ Properly type the refs record
    const sectionRefs = useRef<Record<string, HTMLDivElement | null>>({});

    // Scroll to the matching section when the ID changes
    useEffect(() => {
        if (activeId && sectionRefs.current[activeId]) {
            sectionRefs.current[activeId]?.scrollIntoView({
                behavior: "smooth",
                block: "start",
            });
        }
    ...
    return (
        <main className="min-h-screen mt-20 bg-gradient-to-b from-gray-50 to-gray-100 p-10 flex justify-center">
            <div className="max-w-5xl w-full bg-white shadow-xl rounded-2xl p-10">
                ...
                <div className="space-y-10">
                    {policies.map((policy) => (
                        <div
                            key={policy.id}
                            id={policy.id}
                            // ✅ correct callback ref — must return void, not a value
                            ref={(el) => {
                                sectionRefs.current[policy.id] = el;
                            }}
                            className="border-b border-gray-200 pb-6 last:border-b-0 scroll-mt-28"
                        >
                            <h2 className="text-2xl font-semibold text-primary-text mb-2">
                                {policy.heading}
                            </h2>
                            {policy.descriptionLine && (
                ...
                <button
                    ...
                    className="mt-10 bg-primary-text text-white px-4 py-2 rounded-lg shadow-md cursor-pointer"
                >
                    ← Back to Policies
                </button>
            </div>
